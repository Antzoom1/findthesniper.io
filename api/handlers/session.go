...
func (s *session) Get(w http.ResponseWriter, r *http.Request, mustBeAuthenticated bool) (*domains.Session, error) {
	...
	if !ok {
		return nil, internal.NewErrorf(internal.ErrorCodeInternal, "%v", ErrInvalidID)
	}

	id, err := ulid.Parse(value)
	if err != nil {
		return nil, internal.WrapErrorf(err, internal.ErrorCodeInternal, "%v", ErrInvalidID)
	}
...
func (s *session) Destroy(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return internal.NewErrorf(internal.ErrorCodeInternal, "%v", ErrInvalidID)
	}
